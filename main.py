...
ADMIN_NOT_CONFIGURED_MESSAGE = (
    "Admin user ID is not configured yet. Use /userid from your admin Telegram account, "
    "then add TELEGRAM_ADMIN_USER_ID to .env."
)
...
def admin_ready() -> bool:
    return TELEGRAM_ADMIN_USER_ID is not None
...
async def chat_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not admin_ready():
        await update.message.reply_text(ADMIN_NOT_CONFIGURED_MESSAGE)
        return
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith("settings:"):
            if not admin_ready():
                await query.answer(ADMIN_NOT_CONFIGURED_MESSAGE)
                await query.message.reply_text(ADMIN_NOT_CONFIGURED_MESSAGE)
                return

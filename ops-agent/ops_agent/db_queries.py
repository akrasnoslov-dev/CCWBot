...
QUERIES: tuple[DbQuery, ...] = (
    ...
    DbQuery(
        ...
        "should_alert, error_reason, raw_input_json, created_at AS analysis_created_at "
        "FROM event_ai_analyses WHERE market_event_id IS NOT NULL "
        ...
        "JOIN latest_analysis current_analysis ON current_analysis.market_event_id = e.market_event_id "
        "JOIN alerts a ON lower(a.symbol) = lower(e.symbol) "
        ...
        "LEFT JOIN market_events previous_event ON previous_event.id = a.market_event_id "
        "LEFT JOIN event_ai_analyses previous_analysis ON previous_analysis.id = a.event_ai_analysis_id "
        "WHERE previous_event.event_key = e.event_key OR ("
        "NULLIF(lower(CASE WHEN left(ltrim(coalesce(current_analysis.raw_input_json, '')), 1) = '{' "
        "THEN current_analysis.raw_input_json::jsonb->>'semantic_family' END), '') IS NOT NULL "
        "AND NULLIF(lower(CASE WHEN left(ltrim(coalesce(current_analysis.raw_input_json, '')), 1) = '{' "
        "THEN current_analysis.raw_input_json::jsonb->>'semantic_family' END), '') = coalesce("
        "NULLIF(lower(CASE WHEN left(ltrim(coalesce(a.numeric_context, '')), 1) = '{' "
        "THEN a.numeric_context::jsonb->>'semantic_family' END), ''), "
        "NULLIF(lower(CASE WHEN left(ltrim(coalesce(previous_analysis.raw_input_json, '')), 1) = '{' "
        "THEN previous_analysis.raw_input_json::jsonb->>'semantic_family' END), ''))) "

...
QUERIES: tuple[DbQuery, ...] = (
    ...
    DbQuery(
        ...
        " UNION ALL SELECT llm_operation_id, call_type, status, "
        "'llm_operation_outcome:' || id::text, created_at FROM llm_operation_outcomes "
        "WHERE llm_operation_id IS NOT NULL AND created_at >= :since AND created_at < :until"
        "), feature_rows AS ("
    ...
    DbQuery(
        ...
        "evidence/db/aggregate_metrics.json",
        "WITH usage_rows AS ("
        "SELECT llm_operation_id, call_type, count(*) AS provider_attempts "
        "FROM llm_usage_logs WHERE created_at >= :since AND created_at < :until "
        "AND llm_operation_id IS NOT NULL AND call_type IN ("
        "'event_analysis', 'market_heartbeat', 'daily_report', 'weekly_report', "
        "'news_intelligence') GROUP BY llm_operation_id, call_type"
        "), feature_source_rows AS ("
        "SELECT llm_operation_id, 'event_analysis' AS feature_kind, status AS feature_status "
        "FROM event_ai_analyses WHERE llm_operation_id IS NOT NULL "
        "AND created_at >= :since AND created_at < :until "
        "UNION ALL SELECT llm_operation_id, 'market_heartbeat', status "
        "FROM market_heartbeats WHERE llm_operation_id IS NOT NULL "
        "AND generated_at >= :since AND generated_at < :until "
        "UNION ALL SELECT llm_operation_id, report_type || '_report', status "
        "FROM market_reports WHERE llm_operation_id IS NOT NULL "
        "AND generated_at >= :since AND generated_at < :until "
        "UNION ALL SELECT llm_operation_id, call_type, status FROM llm_operation_outcomes "
        "WHERE llm_operation_id IS NOT NULL AND created_at >= :since AND created_at < :until"
        "), feature_rows AS ("
        "SELECT llm_operation_id, feature_kind, max(feature_status) AS feature_status "
        "FROM feature_source_rows GROUP BY llm_operation_id, feature_kind"

...
QUERIES: tuple[DbQuery, ...] = (
    ...
    DbQuery(
        ...
        "CASE WHEN a.numeric_context IS NOT NULL AND a.numeric_context LIKE '{%' "
        "THEN NULLIF(a.numeric_context::jsonb ->> 'analysed_window_change_percent', '')::numeric "
        "ELSE NULL::numeric END AS analysed_window_change_percent, "
        "CASE WHEN a.numeric_context IS NOT NULL AND a.numeric_context LIKE '{%' "
        "THEN md5(coalesce((a.numeric_context::jsonb -> 'stable_related_news_ids')::text, "
        "coalesce(eaa.related_news_ids, '[]'))) ELSE md5(coalesce(eaa.related_news_ids, '[]')) "
        "END AS stable_news_set_hash "
    ...
    DbQuery(
        ...
        "THEN md5(coalesce((a.numeric_context::jsonb -> 'stable_related_news_ids')::text, "
        "coalesce(eaa.related_news_ids, '[]'))) ELSE md5(coalesce(eaa.related_news_ids, '[]')) "
        "END AS stable_related_news_ids_hash "
        "FROM alerts a JOIN market_events me ON me.id = a.market_event_id "
        "LEFT JOIN alert_delivery_outcomes ado ON ado.alert_id = a.id "
        "LEFT JOIN event_ai_analyses eaa ON eaa.id = a.event_ai_analysis_id "
        "WHERE a.alert_type = 'event_alert' AND a.status = 'sent' "

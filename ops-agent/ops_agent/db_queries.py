...
QUERIES: tuple[DbQuery, ...] = (
    ...
    DbQuery(
        ...
        "WITH usage_rows AS ("
        "SELECT llm_operation_id, call_type, max(provider) AS provider, max(model) AS model, "
        "max(status) AS attempt_status, max(coalesce(error_reason, 'none')) AS error_reason, "
        ...
        "coalesce(u.call_type, f.feature_kind) AS call_type, u.provider, u.model, "
        "coalesce(u.attempt_status, 'missing_usage_telemetry') AS attempt_status, "
        "coalesce(u.error_reason, 'missing_usage_telemetry') AS error_reason, "

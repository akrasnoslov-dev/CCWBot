...
- For production forensic SQL, connect only through the SSH tunnel with `ccwbot_investigator`. Verify the session is read-only before evidence queries. If a required table returns `permission denied`, stop and report the missing grant; never switch to the application/admin role or modify privileges from the investigation session.

...
- `/subscribe` creates a Telegram Stars invoice link for a recurring Premium subscription.
  ...
  it does not prove the Telegram recurring subscription is still active. `/subscribe` can still
  create another invoice for users with active paid access, and a new payment extends access
  from the current paid access date.

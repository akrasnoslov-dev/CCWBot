...
from bot.observability.redaction import collect_secret_values, redact_message
...
class RedactingFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        return redact_message(super().format(record), self._secret_values)
...
def _redact(message: str, secret_values: tuple[str, ...]) -> str:
    return redact_message(message, secret_values)
...
def build_sanitized_log_exports() -> list[LogExport]:
    ...
    for path in get_available_log_export_paths():
        ...
        content = redact_message(text, secret_values).encode("utf-8")

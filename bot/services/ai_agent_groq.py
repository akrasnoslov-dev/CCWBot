...
def build_event_analysis_prompt(input_payload: dict) -> str:
    return (
        ...
        "LLM owns market significance. Do not invent backend price thresholds. market.chg_window "
        "is primary short-term context and market.chg24h is broader context.\n"
...
def build_market_heartbeat_prompt(input_payload: dict) -> str:
    return (
        ...
        "related_news_ids must be candidate_news IDs. Do not repeat the current price, exact price "
        "values, or exact percentage values when a directional summary is sufficient. No hard "
        "financial advice.\n\n"
        "Input JSON:\n"
        f"{_json_dumps(input_payload)}"
    )

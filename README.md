...
- `TELEGRAM_ADMIN_USER_ID` is used for admin permission checks (optional at first startup).
...
1. Start the bot with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` configured (you may leave `TELEGRAM_ADMIN_USER_ID` unset initially).
2. From your intended admin Telegram account, send `/userid` to the bot.
3. Copy that value into `.env` as `TELEGRAM_ADMIN_USER_ID`.
4. Restart the bot.

If `TELEGRAM_ADMIN_USER_ID` is not configured yet, admin-only commands reply with setup guidance.
